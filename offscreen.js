...
async function captureTab(tabId, streamId, panning, volume, eq) {
    ...
    tabAudio[tabId] = { context, source, gainNode, panNode, filters, stream };
}
...
function releaseTab(tabId) {
    ...
    // Tracks keep the underlying tab-capture session alive even after the
    // AudioContext is closed; without stopping them, Chrome still considers
    // the tab "actively captured" and a subsequent getMediaStreamId for the
    // same tab can fail or stall.
